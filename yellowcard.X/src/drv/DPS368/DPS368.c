...
const uint32_t dps368Cal_scalingFactors[] = {
                                             0x00080000,  //No oversampling
...
uint32_t initializeDPS368(DPS368_precision_t presOversample, DPS368_background_t presMeasureRate, DPS368_precision_t tempOversample, DPS368_background_t tempMeasureRate, uint32_t enableFIFO)
{
...
uint32_t setModeDPS368(DPS368_sensorMode_t newMode)
{
...
DPS368_resultStatus_t getResultStatusDPS368()
{
    uint8_t dataBuffer = 0x08;  //Address of the MEAS_CFG register on the sensor
    if (!readFromI2C(DPS368_I2C_ADDR, &dataBuffer, 0x00000001, 0x00000001)) return NO_RESULTS;  //Attempt to read the contents of the MEAS_CFG register on the sensor

    return (DPS368_resultStatus_t) (dataBuffer & 0x30) >> 0x00000004;  //Return the obtained measurement status enum
}
